...
# Script to sync production environment variables to preview in Vercel
# Excludes NEXTAUTH_URL which should be different between environments

echo "🔄 Syncing production environment variables to preview..."
echo ""
...
echo "📋 Variables to sync from production to preview:"
printf '%s\n' "${PROD_ONLY_VARS[@]}"
echo ""
...
for var in "${PROD_ONLY_VARS[@]}"; do
    ...
    if [ -n "$value" ]; then
        ...
        echo "Setting $var in preview..."
        echo "$value" | vercel env add "$var" preview --force 2>/dev/null || echo "  ⚠️  Error setting $var"
    else
